...
export default function SchedulePublicEmployeeView({ schedule }) {
  ...
  const scheduleControls = (
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-end gap-2 w-full">
      {verified && (
        <>
          <button
            ...
            className={`w-full sm:w-auto inline-flex justify-center sm:inline-flex items-center gap-2 border-[2px] border-black rounded-lg font-black uppercase text-xs px-4 py-2 transition-colors shadow-[2px_2px_0px_0px_#000] ${viewMode === "team" ? "bg-[#dcfce7] text-black" : "bg-white text-black hover:bg-[#f3f4f6]"}`}
          >
            <Users size={14} /> Jadwal Tim 
          </button>
          <button
            onClick={() => setViewMode("personal")}
            className={`w-full sm:w-auto inline-flex justify-center sm:inline-flex items-center gap-2 border-[2px] border-black rounded-lg font-black uppercase text-xs px-4 py-2 transition-colors shadow-[2px_2px_0px_0px_#000] ${viewMode === "personal" ? "bg-[#dbeafe] text-black" : "bg-white text-black hover:bg-[#f3f4f6]"}`}
          >
            <User size={14} /> Detail Pribadi
          </button>
          <button
            onClick={handleReset}
            className="w-full sm:w-auto inline-flex justify-center sm:inline-flex items-center gap-2 bg-[#fde047] text-black border-[2px] border-black rounded-lg font-black uppercase text-xs px-4 py-2 hover:bg-[#facc15] transition-colors shadow-[2px_2px_0px_0px_#000]"
          >
            Ganti Nama
          </button>
        </>
      ...
      <button
        onClick={handlePrint}
        className="w-full sm:w-auto inline-flex justify-center sm:inline-flex items-center gap-2 bg-[#fde047] border-[2px] border-black rounded-lg font-black uppercase text-xs px-4 py-2 hover:bg-[#facc15] transition-colors shadow-[2px_2px_0px_0px_#000]"
      >
  ...
  if (viewMode === "personal") {
    ...
    <>
      ...
      <div className="min-h-screen bg-[#fafaf9] flex flex-col print-full">
        {/* Header */}
        <header className="no-print bg-[#111827] border-b-[3px] border-black px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div className="bg-white border-[3px] border-black rounded-2xl shadow-[4px_4px_0px_0px_#000] overflow-hidden team-print-schedule">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-xs border-collapse team-print-table">
                <thead>
                  <tr className="bg-[#f8fafc] border-b-[2px] border-black">
                    <th className="md:sticky left-0 z-20 bg-[#f8fafc] px-3 py-2 text-left font-black uppercase tracking-wider border-r-[2px] border-black min-w-[220px]">
                      Nama Karyawan
                    </th>
                    {Array.from({ length: totalDays }, (_, i) => {
                      const dayNum = i + 1;
                  ...
                  {schedule.rows.map((row) => {
                    ...
                    return (
                      <tr key={row.id} className={`border-b border-gray-100 ${matched ? "bg-yellow-50" : "hover:bg-gray-50"}`}>
                        <td className={`md:sticky left-0 z-10 px-3 py-2 font-black border-r-[2px] border-black ${matched ? "bg-yellow-100" : "bg-white"}`}>
