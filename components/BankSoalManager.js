...
export default function BankSoalManager() {
  ...
  const [sharingId, setSharingId] = useState(null); // ID materi yang sedang diproses share
  ...
  return (
    <div className="space-y-8 p-4 md:p-8 bg-[#fafaf9] min-h-screen text-gray-900 font-sans">
      
      {/* HEADER: Soft Brutalism */}
      <div className="flex flex-col md:flex-row items-start md:items-center gap-5 animate-fade-in-up border-b-[3px] border-black pb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        {/* FORM SECTION */}
        <section className="lg:col-span-5 bg-white border-[3px] border-black shadow-[6px_6px_0px_0px_#000] rounded-xl overflow-hidden h-fit">
          <div className="bg-[#e0e7ff] text-black p-4 border-b-[3px] border-black flex items-center justify-between">
            ...
            <div className="border-t-[3px] border-dashed border-gray-300 pt-5">
              ...
              <div className="space-y-4 mb-4">
                {form.items.map((item, index) => (
                  <div key={index} className="border-[3px] border-black rounded-xl p-4 bg-[#f8fafc] relative">
                    ...
                    <div className="space-y-3">
                      <input
                        ...
                        className="w-full border-b-[3px] border-black p-2 font-bold bg-transparent outline-none focus:bg-white transition-colors"
                        placeholder="Tulis Pertanyaan..."
                      />
                      <textarea
                        value={item.answer} onChange={(e) => setItemField(index, "answer", e.target.value)}
                        rows={2}
                        className="w-full border-[3px] border-transparent focus:border-black p-2 font-medium bg-white rounded-lg outline-none resize-none transition-all"
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                {materials.map((material) => (
                  <div key={material.id} className="bg-white border-[3px] border-black rounded-xl p-5 shadow-[4px_4px_0px_0px_#000] hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_#000] transition-all flex flex-col sm:flex-row gap-4 justify-between">
                    ...
                    <div className="flex flex-col sm:flex-row items-start gap-2 shrink-0 justify-center">
                      <button 
                        onClick={() => openPreview(material)} 
                        className="bg-[#bfdbfe] border-[3px] border-black rounded-lg font-black uppercase text-[10px] py-2 px-3 hover:bg-[#93c5fd] shadow-[2px_2px_0px_0px_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all flex items-center justify-center gap-1.5"
                      >
                        <Eye size={14} /> Detail
                      </button>

                      {/* TOMBOL SHARE — hanya untuk materi TEAM */}
                      {material.accessStatus === "TEAM" && (
                        material.publicSlug ? (
                          <div className="flex flex-col gap-1.5">
                            <button
                              onClick={() => copyToClipboard(material.publicSlug)}
                              className="bg-[#dcfce7] border-[3px] border-black rounded-lg font-black uppercase text-[10px] py-2 px-3 hover:bg-[#bbf7d0] shadow-[2px_2px_0px_0px_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all flex items-center justify-center gap-1.5"
                            >
                            ...
                            <button
                              ...
                              className="bg-[#fee2e2] border-[3px] border-black rounded-lg font-black uppercase text-[10px] py-2 px-3 hover:bg-[#fecaca] shadow-[2px_2px_0px_0px_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                            >
                              {sharingId === material.id ? <Loader2 size={14} className="animate-spin" /> : <Link2Off size={14} />} Cabut Akses
                            </button>
                          </div>
                        ) : (
                          <button
                            onClick={() => handleShare(material)}
                            disabled={sharingId === material.id}
                            className="bg-[#fef9c3] border-[3px] border-black rounded-lg font-black uppercase text-[10px] py-2 px-3 hover:bg-[#fde047] shadow-[2px_2px_0px_0px_#000] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                          >
                            {sharingId === material.id ? <Loader2 size={14} className="animate-spin" /> : <Share2 size={14} />} Bagikan
                          </button>
                        )
                      )}
